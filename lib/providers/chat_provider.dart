...
class ChatProvider with ChangeNotifier {
  ...
  // Геттер для получения состояния ошибки авторизации
  bool get hasAuthError => _hasAuthError;
  // Геттер для получения сообщения об ошибке авторизации
  ...
  Future<void> _initializeProvider() async {
    try {
      ...
      // Инициализация API клиента из настроек
      await _api.initializeFromSettings();
      _log('API client initialized from settings');
  ...
  Future<void> updateApiSettings() async {
    try {
      _log('Updating API settings...');
      // Переинициализация API клиента с новыми настройками
      await _api.initializeFromSettings();
      // Перезагрузка моделей и баланса
      await _loadModels();
      await _loadBalance();
      _log('API settings updated successfully');
    } catch (e) {
      _log('Error updating API settings: $e');
    }
  }
  ...
  // Метод загрузки баланса пользователя с обработкой ошибок авторизации
  Future<void> _loadBalance() async {
    try {
      // Получение результата баланса с обработкой ошибок
      final result = await _api.getBalanceWithResult();
      ...
      if (result.isSuccess) {
        ...
        if (result.errorType == ApiErrorType.unauthorized) {
          _balance = 'ошибка авторизации';
          _hasAuthError = true;
          _authErrorMessage = result.errorMessage;
        } else if (result.errorType == ApiErrorType.networkError) {
          _balance = 'ошибка сети';
          _hasAuthError = true;
          _authErrorMessage = result.errorMessage;
        } else {
          _balance = 'ошибка подключения';
          _hasAuthError = true;
          _authErrorMessage = result.errorMessage;
        }
      }
      ...
      _balance = 'ошибка авторизации';
      _hasAuthError = true;
      _authErrorMessage = 'Ошибка соединения';
      notifyListeners();
    }
  }
  ...
  Future<void> sendMessage(String content, {bool trackAnalytics = true}) async {
    ...
    if (content.trim().isEmpty || _currentModel == null) {
      return;
    }
  ...
  // Метод создания временного API клиента для тестирования
  OpenRouterClient createTempApiClient({
    required String provider,
    required String apiKey,
    required int maxTokens,
    required double temperature,
  }) {
    final tempClient = OpenRouterClient();
    tempClient.initializeWithSettings(
      provider: provider,
      apiKey: apiKey,
      maxTokens: maxTokens,
